...
{   
    ...
    {
        $pass = User::where('email', $request->username)->get('password');
        $pass=User::where('username', $request->username)->get('password');
        // dd($request->password);
        if ($pass) {
    ...
    public function signup(Request $request)
    {
        ...
        if($user) return response("Username existed", 400);
        $data=$request->all();
        $data['password']=Hash::make($request->password);
        $data['photo']="https://api.minimalavatars.com/avatar/random/svg";
        // $request->merge(['password' => Hash::make($request->password)]);
        // dd($request->password,$request->all());
